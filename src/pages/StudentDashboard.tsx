  ...
  const aiMutation = useMutation({
    mutationFn: async (prompt: string) => {
      const res = await fetch('/api/ai/generate', {
        ...
        body: JSON.stringify({
          prompt,
          context: { results, stats, studentName: user?.name }
        })
      });
  ...
  return (
    <DashboardLayout>
      <div className="mb-10 text-center md:text-left">
        <h1 className="text-3xl font-serif font-bold text-navy mb-2">Welcome back, {user?.name?.split(' ')[0]}</h1>
        ...
        <div className="lg:col-span-2 space-y-8">
          ...
          <div className="card p-0 overflow-hidden">
             <div className="p-8 border-b border-border flex justify-between items-center">
                ...
                <Link to="/papers" className="text-accent text-sm font-bold hover:underline flex items-center gap-1">
              ...
              <div className="flex-1 overflow-y-auto mb-6 space-y-6 scrollbar-hide">
                 {!aiResponse && !aiMutation.isPending && (
                    <div className="h-full flex flex-col items-center justify-center text-center space-y-4 px-4 opacity-60">
                       ...
                       <p className="text-sm font-medium">Ask Gemini for a personalized study plan based on your recent exam performance.</p>
