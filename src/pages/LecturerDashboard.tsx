...
const LecturerDashboard = () => {
  ...
  const { data: sessionsData, isLoading: sessionsLoading, isError: sessionsError } = useQuery({
    ...
    queryFn: () => fetch('/api/sessions?page=1&limit=10', {
      headers: { 'Authorization': `Bearer ${localStorage.getItem('markai_token')}` }
    }).then(res => {
  ...
  return (
    <DashboardLayout>
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-10">
        <div>
          <h1 className="text-3xl font-serif font-bold text-navy mb-2">Welcome, {user?.name}</h1>
            ...
            <div className="flex-1 overflow-y-auto mb-4 space-y-4">
              {!aiResponse && !aiMutation.isPending && (
                <div className="h-full flex flex-col items-center justify-center text-center opacity-40">
                  ...
                  <p className="text-sm font-medium">Ask Gemini for insights about your classes, marking criteria, or student trends.</p>
                </div>
              )}
