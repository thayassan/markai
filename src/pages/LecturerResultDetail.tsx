...
const LecturerResultDetail = () => {
  ...
  const { data: result, isLoading, isError } = useQuery({
    ...
    queryFn: () => fetch(`/api/results?sessionId=${sessionId}&studentId=${studentId}`, {
  ...
  const overrideMutation = useMutation({
    mutationFn: async ({ resultId, questionId, mark, note }: any) => {
      const res = await fetch(`/api/results/${resultId}/override`, {
        method: 'PATCH',
        headers: {
    ...
    <DashboardLayout>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-12">
         ...
         <div className="card p-6 bg-bg flex items-center justify-center border-dashed border-2 border-border">
            <button className="flex items-center gap-2 text-text-muted hover:text-navy transition-all text-xs font-bold uppercase tracking-widest">
               <RefreshCcw size={14} /> Re-evaluate with AI
            </button>
         </div>
      </div>
