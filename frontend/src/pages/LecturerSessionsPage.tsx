...
const LecturerSessionsPage = () => {
  ...
  const { data: sessionsData, isLoading, isError } = useQuery({
    ...
    queryFn: () => {
      ...
      return apiFetch(url.toString()).then(res => {
                    ...
                    <td className="px-8 py-5 text-right">
                      <div className="flex justify-end gap-2">
                        <button 
                          ...
                          className={cn(
                            "p-2 rounded-lg transition-all text-text-muted hover:bg-red-50 hover:text-red-500",
                            deleteMutation.status === 'pending' && deleteMutation.variables === session.id && "animate-pulse"
                          )}
                          title="Delete Session"
                        >
                          {deleteMutation.status === 'pending' && deleteMutation.variables === session.id ? (
            ...
            >
              <div className="card p-8 sticky top-8">
                ...
                <div className="space-y-3">
                  ...
                  <button 
                    onClick={(e) => handleDelete(e, selectedSession.id, selectedSession.name)}
                    disabled={deleteMutation.status === 'pending'}
                    className="btn-ghost w-full flex items-center justify-center gap-2 text-xs text-red-500 hover:bg-red-50 mt-2"
                  >
