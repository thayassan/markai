...
const LandingPage = () => {
  ...
  const dashboardPath = user ? (user.userType === 'STUDENT' ? '/dashboard' : user.userType === 'LECTURER' ? '/lecturer/dashboard' : '/admin/dashboard') : '/register';

  const [testimonials, setTestimonials] = React.useState([]);

  useEffect(() => {
    const savedFeedback = JSON.parse(safeGetItem('userFeedback') || '[]');
    const initial = [
      ...
      { name: 'Mr. Sanjay Mehta', role: 'Mathematics Lead', inst: 'Colombo Int. University', quote: 'The accuracy of the method marks is what impressed me most. It understands the steps, not just the answer.' },
      { name: 'Fatima Al-Rashidi', role: 'IB Coordinator', inst: 'IB World University Dubai', quote: 'Our students love the instant feedback. They no longer have to wait 2 weeks to know where they went wrong.' },
    ...
    setTestimonials([...savedFeedback, ...initial]);
  }, []);
        ...
        <div className="max-w-7xl mx-auto px-6">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {[
              ...
              { role: 'Universities', color: 'navy', items: ['Standardize marking across faculties', 'University-wide performance monitoring', 'Reduce teacher burnout & turnover', 'Data-driven intervention planning'] }
            ].map((p) => (
