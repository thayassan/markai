...
export class ErrorBoundary extends React.Component<Props, State> {
  ...
  public componentDidCatch(error: Error, errorInfo: ErrorInfo) {
    ...
    // Recovery for infinite redirect loops (Common in Safari when history is throttled)
    if (error.message.includes('replaceState') || error.message.includes('100 times')) {
      console.warn('Redirect loop detected. Clearing storage and resetting...');
      localStorage.clear();
